...
const post = (request, c) => {
  try {
    ...
    const doc = parser.parseFromString(`<div>
      <span style="color: red">Etwas ist schief gelaufen. Bitte starte Glubschi neu...</span>
      <br>
      <br>
      ${e.message}
    </div>`, 'text/html');
  ...
  post({
    ...
    const run = async src => {
      ...
      try {
        ...
        if (o.text.trim() === '') {
          result.textContent = 'Keine Inhalte gefunden.';
        }
        else {
...
document.getElementById('copy').addEventListener('click', e => {
  ...
  navigator.clipboard.writeText(value).catch(() => {
    ...
    e.target.value = 'Fertig';
    setTimeout(() => e.target.value = 'Kopieren', 1000);
  });
});
...
document.getElementById('expand').addEventListener('click', e => {
  ...
  e.target.value = e.target.dataset.mode === 'expand' ? 'Zuklappen' : 'Ausklappen';
  post({
    method: 'resize',
